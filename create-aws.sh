...
instanceID=$(aws ec2 describe-instances --filters instance.group-name EC2deployLinuxInstance --query 'Reservations[*].Instances[*].[InstanceId]' --output text)
...
volumeID=$(aws ec2 describe-volumes --filters attachment.instance-id=$instanceID --query \"Volumes[*].[VolumeId]\" --output text)
